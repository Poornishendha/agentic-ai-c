...
# ---------------------------------------------------------------------------
# 2. CHUNK —  character-based splits
# ---------------------------------------------------------------------------
...
# It uses that search engine to find the top matching code chunks
#  whenever the agent asks a question.
   def _get_relevant_documents(
       self, query: str, *, run_manager: CallbackManagerForRetrieverRun
   ) -> list:
       tokens = query.lower().split()
       # BM25 has score every chunk
        #Chunk 0 -> 8.5
        #Chunk 1 -> 2.1
        #Chunk 2 -> 15.3
        #Chunk 3 -> 6.7
       scores = self.bm25.get_scores(tokens)
       # sorts those indices based on the scores
       top_indices = sorted(range(len(scores)), key=lambda i: scores[i], reverse=True)[: self.k]
       return [self.docs[i] for i in top_indices]

# It creates the search engine
